...
打开首页就是知乎热榜；点任意一条热榜题，我们把该题的社区回答提炼成十几条判断；每条判断展开成一条**取向光谱**：两端由判断自身的内容定义，每一档上站着真实答主（昵称 + 认证 + 权威级 + 可回跳原文的原话）。

**产品不评判对错，只呈现分布。**

> 🔒 **产品入口只有一个：热榜**（2026-09-13 拍板）。没有「输入问题文字去搜」的入口——自由文本搜索召回不稳定，要靠救援合并兜底，复杂度高而收益不确定；热榜题每天 ≤30 道、可提前预生成，体验与质量都更可控。
...
| # | 约束 | 含义 |
|---|---|---|
| 1 | **热榜题打开即出结果** | 渲染路径**零实时生成调用**，全部读按日分析快照；00:30 全量预生成（单题预算 600s，没有用户在等，可以做得更充分）。带外直接访问未预置 qid 才进按需生成（T1 生成态），快照落地后同样秒开 |
...
| 界面 | 用户看到什么 |
|---|---|
| **N1 首页** | 知乎热榜「光幕」弹幕道 —— 热榜题横向流动，点任意一条直接进问题页（**无输入框**）；登录知乎后，你收藏过的题会多一枚「与你有关」标记 |
| **N2 问题页** | 十几条判断化作钉在光幕上的「钉子」，每颗标注「N 人表态 · 分歧度 X」；首屏 4 条，其余折叠；与 N1 同一套「与你有关」标记 |
| **N3 判断展开** | 判断展开成一条五档取向光谱：两端是判断自己的语义轴（不预设正反），每档站着真实答主；点答主看原话 + 归位理由 + 「查看知乎原文」；底部「看山解读」给出中立综述 |
| **T1 生成中** | 按需生成题的四阶段进度（提取 → 归并 → 取向 → 综述），诚实告知当前阶段与已处理样本数；正常产品路径不触达 |
...
| 变量 | 必填 | 默认值 | 用途 |
|---|---|---|---|
| `ZHIHU_ACCESS_SECRET` | ✅ | — | 开放平台调用方凭证，热榜 / 搜索全部用它做 Bearer 鉴权 |
...
| `ANALYSIS_JOB_TIMEOUT_SEC` | 可选 | `600` | 单 job 总时限，超时置 `timeout`。2026-09-13 起从 180 放宽：产品入口只剩热榜，单题不再受交互延迟约束 |
...
| `PIPELINE_QUERY_VARIANTS` | 可选 | `4` | 单题检索路数上限（1–4）= 原句 + LLM 扩写问法；直接决定样本池大小（≤10 → ≤40 条）。设 `1` 退回单路搜索 |
| `ZHIHU_FAV_SCAN_LISTS` | 可选 | `5` | 增强层 · 扫几个收藏夹（收藏夹按新→旧排序，只需最近若干条） |
| `ZHIHU_FAV_SCAN_PAGES` | 可选 | `1` | 增强层 · 每个收藏夹扫几页（页长见下）；短页即停 |
| `ZHIHU_FAV_PAGE_SIZE` | 可选 | `50` | 增强层 · 收藏夹内容单页条数 |
| `USER_SIGNAL_TTL_SEC` | 可选 | `600` | 增强层 · 同一会话内不重复扫收藏的缓存时长 |
...
登录后提供两样东西：**登录态**（`/auth/zhihu/status`，前端据它显示「已登录 / 退出」）与**「与你有关」标记**（`/me/related`）。标记只承认**两种 id 级精确匹配**：

| 原因 | 依据 | 说明 |
|---|---|---|
| `question_favorited` | 收藏夹里 `ContentType=question` 的条目 → `/question/<qid>` | 你收藏过**这道题** |
| `answer_favorited` | 收藏夹里回答条目的 `/answer/<id>` ∩ 快照内答主原链 | 你收藏过**这题里被写进光谱的回答** |

> ⚠️ **「你关注了这道题」做不到**：开放平台没有该接口（`user_followees` 是关注的**用户**，不是关注的问题），也没有回答详情类接口可补。因此本项目**刻意不做**按昵称匹配「你关注的人是不是这题的答主」——重名即误判。
> **只漏报、不误报**：一条回答若在归类阶段被判为「不涉及任何议题」，它不会进快照，收藏它也就点不亮标记。缺省不渲染标记，因为「没标记」不等于「与你无关」。
...
| `GET` | `/me/related` | 增强层 · 当日热榜题里「与你有关」的那些（`?qid=` 可额外判定某个带外题）。未登录 → `200` + `{authorized:false}`，**不是 403** |
...
| `GET` | `/auth/zhihu/status` | 增强层 · 登录态（`configured` / `authorized` / `expiresAt`），不产生任何知乎调用 |
| `POST` | `/auth/zhihu/logout` | 增强层 · 清服务端会话与本地用户信号缓存 |
...
- **生成耗时控制**：固定 worker 持续补位；归并模型只返回来源编号，本地还原回答和原话。获取内容/提取/归并/取向/综述的阶段上限分别为 240/150/120/150/60 秒，两轮路径的主题发现/逐条归类为 120/150 秒，同时受整题剩余时间约束。默认 600 秒整题预算（标定基准，见 `budget.ts` 的 `BUDGET_REFERENCE_MS`）下，各阶段分别预留 300/150/60/30/5 秒供后续阶段使用；配置更短整题时限时预留时间同比缩短，更长则不超过标定值。提取和取向保留超时前已完成结果；全部失败仍为 failed。综述最多一次调用，失败或时间不足时省略综述并保存分析。
- **快速两轮分析**：LLM 模式先把本题检索到的全部回答一次性喂给模型，提炼 3–8 个有证据的共同议题和固定语义轴；再为每条回答并行调用分类模型，允许一条回答命中多个议题，也允许明确返回「不涉及任何议题」。服务端只接受来源编号、原文连续片段和已知议题，未知编号、伪造引文、冲突归位会被丢弃或标记为失败，不会被默认为中立票。回答归类完成后先写入 `ready`，刘看山综述独立后台补写；前端先展示光谱并提示综述生成中。
- **重试和扩写**：提取/归并/取向/救援/扩写每次最多 2 个 HTTP 尝试，主备共用次数和总截止时间；限流等待、退避、响应体读取和校验都计入。管线扩写总上限 15 秒（失败降级为仅原句）；可选救援总上限 10 秒，失败保留已取得的同题来源。
