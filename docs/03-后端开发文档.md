...
- 全局限速：令牌桶（默认 60 req/min，`LLM_RPM_LIMIT` 可调），管线共享，防打爆
...
> **为什么单次只能拿 10 条**：`zhihu_search` 的 `Count` 服务端上限就是 10（`>10` 自动截断为 10，`<=0` 回退为 10）。这正是「3 个 Query 变体去重合并」方案成立的前提——上限是**单次**的，不是总量上限。
...
归并：①只有核心主张、对象、条件和价值取舍都相同、仅措辞不同的判断才合并；②结论方向、适用对象/人群、前提条件、时间范围、因果解释不同，或同一回答中的主张与让步/反例不同，必须保留为不同判断；③代表表述要具体到可以单独被支持或反驳，最多输出 15 条时优先覆盖不同子议题和冲突关系，避免为了减少条数而过度合并；④同一 `answerId` 可以出现在多个判断中，表示一条回答确实包含多个观点。
...
| 字段 | 统计对象 | 口径 |
|---|---|---|
| `sampleCount`（分析级） | 回答**条数** | 3 次 Query 变体去重合并后实际纳入内容池的回答数；专栏文章不计入；UI 用于头部声明「基于 N 条回答」 |
...
| `GET /auth/zhihu/callback` | 条件模块 · 读取回调参数 **`authorization_code`**（兼容 `code`）→ 后端换 token；**不留存、只取公开范围数据**，会话 cookie |
...
**GET 的副作用边界（全系统唯一一处读请求带副作用）**：快照未命中时，GET 会隐式触发懒生成——先参与 job 的 `INSERT` 竞争（§6.3），抢到所有权则启动 runner，随后返回 202。理由：冷门题点开即出结果，前端不必先 POST 才拿得到进度。幂等性由 `UNIQUE(date, qid)` 保证，重复 GET 不会重复生成；`ready` 之后退化为纯读。
...
| `timeout` | 单 job 超过总时限（默认 180s） | — | ✅ | 生成超时，可重试 |
...
- `cron` 模块内置进程（Bun 定时器计算下一个 `Asia/Shanghai 00:30` 的毫秒差，避免引入 cron 依赖），也可外置 `crontab curl POST /internal/pregenerate`（带 `INTERNAL_TOKEN`）
- 流程：拉热榜 → 过滤 question → 逐题跑管线（**题间串行、题内扇出**）→ 成功写 ready；**单题失败标记 `failed` 并继续，不阻塞队列**（失败题不进 hot 列表，其余题目照常）
- cron **不对 failed 做无限重试**：最多在队列末尾重跑一次仍失败即放弃，等用户显式重试或次日新日期键重建
- 懒生成兜底保证用户侧永不白屏
...
services:
  web:        # Caddy：静态 Vue 产物 + 反代 /api → server
    ...
    volumes: ["./apps/web/dist:/srv", "./Caddyfile:/etc/caddy/Caddyfile"]
  server:
    build: ./apps/server
    environment:
      - TZ=Asia/Shanghai
    ...
    volumes: ["./data:/app/data"]   # SQLite 单文件
...
- 秒开验收标准：热榜页与预置题 **TTFB < 200ms、渲染路径无任何实时生成调用**（懒生成路径除外）
