...
**快照决定入口状态**：热榜题直接 200 秒开；未预置 qid（带外访问）由 GET 隐式触发按需生成（后端抢到 job 所有权即开工），返回 202 进 T1——「零实时生成调用」承诺的是**热榜题的渲染路径**。轮询的就是这个 GET 本身，**没有第二个端点**。
...
:root {
  ...
  --line:       #E2DFD8;  /* 描边（授权按钮/引用卡） · 0.886,0.875,0.847 */
...
|---|---|
| 页边距 | 48px |
| N1：品牌行 y0 h100 · 光幕区 y100 h600 · 声明行 y700 h100（**2026-09-13 起无输入行**） | |
...
> **与设计稿的差异**：早期设计稿在光幕下方有「输入行 y700 h100」（InputBox 高 52 · 圆角 26 + CTA「看看分歧」）。产品已于 2026-09-13 砍掉用户输入，入口只剩热榜，该行连同 `SearchBar` 组件一并移除。
...
getZhihuStatus(): Promise<ZhihuAuthStatusResp>             // 增强层：登录态（不产生知乎调用）
getRelated(qid?): Promise<RelatedResp>                     // 增强层：与你有关（收藏过的题/回答）
logoutZhihu(): Promise<void>                               // 增强层：退出登录
goZhihuAuthorize(): void                                   // 增强层：整页跳转授权
...
- 增强层也能离线看：`?mock=authorized` 模拟「已登录 + 有标记」（mock 会给出两条示例标记），不带该参数则是未登录态
- mock 数据字段与 `packages/contract` 的 zod schema 校验一致，后端联调时零改动

## 7.1 增强层 · 登录态与「与你有关」标记

`stores/user.ts` 一份状态供 N1 与 N2 共用：`authorized` / `marks(qid→reasons)` / `degraded`，TTL 5 分钟（比后端 `USER_SIGNAL_TTL_SEC` 短，不会长期用陈旧标记）。

- **一次拉完**：`getZhihuStatus()` 与 `getRelated()` 并行；`getRelated` 的 `authorized` 就是登录态，不依赖 status 才能决定按钮文案。
- **安静降级**：任何失败都退成「未登录 / 无标记」，绝不影响热榜与问题页的渲染。
- **OAuth 回跳**：`/?oauth=success` 时强制 `reloadAfterOauth()` 并把 `oauth` 参数从地址栏摘掉；`oauth=error` 只提示一次，不报错。
- **标记文案**：`question_favorited` → 「你收藏过这道题」；`answer_favorited` → 「你收藏过这题里的回答」；两者同时命中取前者（更具体）。

**视觉与几何（重要）**：N1 的标记是**绝对定位在卡片下内边距区**的小胶囊（`right:12 / bottom:2 / h:18`），**刻意不进文档流**：

1. 弹幕道只有 112px 高，卡片进流会撑到 116px 被 `overflow:hidden` 裁切；
2. 标记是热榜之后**异步**到达的，进流会让整条跑马灯重排抖动 —— 与「避免明显错落/抖动」的设计要求直接冲突。

标题最多 2 行 = 20+48=68px，卡高恒为 88px，标记落在 68→86px 的下内边距里，与标题不重叠。N2 头部空间充裕，用同一套圆点+描边胶囊语言，正常参与文档流。

**缺省即静默**：没有标记时不渲染任何元素，也不写「与你无关」之类否定文案（后端只保证「命中才输出」，缺省是「无从判断」）。
...
- **性能**：快照 JSON 一次拉全（<100KB 量级）；热榜题渲染路径零实时生成调用，仅带外未预置 qid 触发按需生成；弹幕动画用 `transform` 走合成层；Canvas 仅在展开态存在
...
│   ├── home/       # HotGrid / HotCard / HonestFooter
...
├── stores/         # analysisStore（状态机 + 单端点轮询）/ userStore（登录态 + 与你有关）
...
| 演示视频 | 录屏：热榜光幕 → N2 钉子 → N3 展开 → 回跳原文，60–90s | 选交加分 |
