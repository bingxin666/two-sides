...
/**
 * 总时限模型（不设轮数上限）：真实管线单题 60–150s+（极端 LLM 慢）。
 * 2026-09-13：后端整题预算放宽到 600s（热榜全量预生成，无用户输入），
 * 这里的等待上限必须盖过它，否则会出现「后端还在跑、前端先报超时」的假失败。
 * 超时只是前端停止等待（docs/03 §6.3「取消」语义）—— 后端任务继续跑，
 * 快照落地后重进即秒开；用户点「重试」走 POST 幂等恢复轮询。
 * 热榜题都是预生成的 ready 快照，T1 本身是边缘路径，且卡片上始终有「取消」。
 */
const POLL_DEADLINE_MS = 11 * 60_000
